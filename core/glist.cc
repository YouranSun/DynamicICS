...
void GLIST::InsertUpd(const int v1, const int v2,
                   std::vector<std::vector<int>>& graph,
                   std::vector<int>& core,
                   int &k_max) {
  // insert the edge
...
void GLIST::InsertRem(const int v1, const int v2,
                   std::vector<std::vector<int>>& graph,
                   std::vector<int>& core,
                   std::vector<std::vector<int> >& core_inc) {
  // insert the edge
  graph[v1].push_back(v2);
  graph[v2].push_back(v1);
  // update mcd
  ...
  if (evicted_[src]) {
    auto tail = -1; // tail
    for (auto v = src; n_ != v; v = node_[v].next) {
      // fprintf(stderr, "core increase: v = %d\n", v);
      ++core[v];
      // fprintf(stderr, "v = %d core = %d\n", v, core[v]);
      ASSERT(core[v] < core_inc.size());
      core_inc[core[v]].push_back(v);
      node_[v].ext = 0;
      tail = v;
      // update mcd
      for (const auto u : graph[v]) {
...
void GLIST::RemoveRem(const int v1, const int v2,
                   std::vector<std::vector<int>>& graph,
                   std::vector<int>& core,
                   std::vector<std::vector<int> >& core_dec) {
  // remove the edge
  graph[v1].erase(std::find(graph[v1].begin(), graph[v1].end(), v2));
  graph[v2].erase(std::find(graph[v2].begin(), graph[v2].end(), v1));
  // update the mcd values
  ...
  for (auto v: changed) core_dec[core[v] + 1].push_back(v);
  for (const int g : garbage_) rank_[g] = 0;
  garbage_.clear();
}
void GLIST::RemoveUpd(const int v1, const int v2,
                   std::vector<std::vector<int>>& graph,
                   std::vector<int>& core,
                   int &k_max) {
  // remove the edge
  ...
  for (const int g : garbage_) rank_[g] = 0;
  while(k_max && head_[k_max] == -1) --k_max;
  garbage_.clear();
}
...
void GLIST::GetAtLeast(int k, std::vector<int> &vec) const{
  for (int i = k; i < root_.size(); ++i) {
    tree_.TraverseToVec(root_[i], vec);
  }
}
}  // namespace core
